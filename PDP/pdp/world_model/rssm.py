...
from pdp.world_model.utils_rssm import RSSMUtils, RSSMContState, RSSMDiscState
...
class RSSM(nn.Module, RSSMUtils):
    ...
    def _build_embed_state_action(self):
        """
        Embedder for RNN: e_t = f_e(z_{t-1}, a_{t-1})
            - the rnn handles encoding the h_{t-1}
    ...
    def _build_temporal_prior(self):
        """
        Transition predictor, z^t ~ p(z^t | h^{t})
    ...
    def _build_temporal_posterior(self):
        """
        Representation predictor, z^t ~ p(z^t | h^{t}, x_emb^t)
    ...
    def rollout_observation(self, seq_len:int, obs_embed: torch.Tensor, action: torch.Tensor, prev_rssm_state):
        ...
        for t in range(seq_len):
            ...
            prior_rssm_state, posterior_rssm_state = self.rssm_observe(obs_embed[t], prev_action, prev_rssm_state)
