...
class RSSMUtils(object):
    ...
    def _init_rssm_state(self, batch_size, device, **kwargs):
        if self.rssm_type  == 'discrete':
            return RSSMDiscState(
                torch.zeros(batch_size, self.stoch_size, **kwargs).to(device),
                torch.zeros(batch_size, self.stoch_size, **kwargs).to(device),
                torch.zeros(batch_size, self.deter_size, **kwargs).to(device),
            )
        elif self.rssm_type == 'continuous':
            return RSSMContState(
                torch.zeros(batch_size, self.stoch_size, **kwargs).to(device),
